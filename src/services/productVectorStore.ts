...
export const initProductCollection = async (
  ...
  console.log(
    `Product embeddings for site ${siteId} managed via PostgreSQL/Prisma`
  );
...
export const upsertProductEmbedding = async (
  ...
  try {
    console.log(
      `Upserting product embedding for product ${productData.product_id} in site ${siteId}`
    );
    ...
    const numericRating =
      productData.rating !== undefined
        ? typeof productData.rating === "string"
          ? parseFloat(productData.rating as any)
          : productData.rating
        : null;
    const numericPrice =
      productData.price_usd !== undefined
        ? typeof productData.price_usd === "string"
          ? parseFloat(productData.price_usd as any)
    ...
    if (existingEmbedding) {
      ...
      await prisma.$executeRaw`
         ...
         SET title = ${productData.title},
             ...
             structured_data = ${
               productData.structured_data
                 ? JSON.stringify(productData.structured_data)
                 : null
             }::jsonb,
      ...
      await prisma.$executeRaw`
         ...
         VALUES (gen_random_uuid(), ${siteId}, ${productData.product_id}, ${
        productData.title
      }, ${productData.url}, ${productData.brand || null}, ${
        productData.category || null
      }, ${numericPrice}, ${numericRating}, ${
        productData.availability || null
      }, ${productData.searchable_text}, ${
        productData.structured_data
          ? JSON.stringify(productData.structured_data)
          : null
      }::jsonb, ${embeddingVector}::vector, NOW(), NOW())
       `;
    }

    console.log(
      `Successfully upserted product embedding for product ${productData.product_id}`
    );
  } catch (error) {
    console.error(
      `Error upserting product embedding for product ${productData.product_id}:`,
      error
    );
    throw error;
  }
...
export const dropProductCollection = async (siteId: string): Promise<void> => {
  try {
    ...
    console.error(
      `Failed to delete product embeddings for site ${siteId}:`,
      error
    );
    throw error;
  }
 ...
 */
export const getProductEmbeddingsCount = async (
  siteId: string
): Promise<number> => {
  try {
    // Use raw SQL to count products with embeddings
    ...
    console.error(
      `Error getting product embeddings count for site ${siteId}:`,
      error
    );
    return 0;
  }
